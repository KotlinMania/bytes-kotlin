# Claude Code Project Instructions — bytes-kotlin

## Project Overview

This is **bytes-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`bytes`](https://crates.io/crates/bytes).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/bytes/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/bytes>
