...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`bytes`](https://crates.io/crates/bytes).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `bytes` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Utilities for working with byte buffers (`Bytes`, `BytesMut`)
...
Original work copyrighted by the upstream `bytes` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
